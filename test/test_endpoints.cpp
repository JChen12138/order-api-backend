...
TEST_CASE("Delete order removes from Redis and database") {
    ...
    auto res = cli.Post("/order/create", body, "application/json");
    CHECK(res != nullptr);
    ...
    auto res2 = cli.Get(("/order/get/" + order_no).c_str());
    CHECK(res2 != nullptr);
    CHECK(res2->status == 200);

    // Step 3: Delete the order
    auto res3 = cli.Delete(("/order/delete/" + order_no).c_str());
    CHECK(res3 != nullptr);
    CHECK(res3->status == 200);

    // Step 4: Confirm deletion (should return 404)
    auto res4 = cli.Get(("/order/get/" + order_no).c_str());
    CHECK(res4 != nullptr);
...
TEST_CASE("Creating order with invalid amount returns 400") {
    ...
    auto res_zero = cli.Post("/order/create", zero_body, "application/json");
    CHECK(res_zero != nullptr);
    CHECK(res_zero->status == 400);

    string negative_body = R"({"amount": -1})";
    auto res_negative = cli.Post("/order/create", negative_body, "application/json");
...
TEST_CASE("Paying for a created order updates state") {
    ...
    auto res_create = cli.Post("/order/create", create_body, "application/json");
    ...
    auto res_pay = cli.Post("/order/pay", pay_body, "application/json");
    CHECK(res_pay != nullptr);
    CHECK(res_pay->status == 200);

    // Step 3: Check status updated
    auto res_get = cli.Get(("/order/get/" + order_no).c_str());
...
TEST_CASE("Paying with invalid or missing order_no returns 400") {
    ...
    auto res_empty = cli.Post("/order/pay", empty_body, "application/json");
    CHECK(res_empty != nullptr);
    CHECK(res_empty->status == 400);

    string missing_body = R"({})";
    auto res_missing = cli.Post("/order/pay", missing_body, "application/json");
